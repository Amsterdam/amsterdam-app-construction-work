...
def project(request):
    ...
    try:
        ...
        project = Projects.objects.filter(pk=data.get('identifier')).first()
        if project is None:
            return Response({'status': False, 'result': message.no_record_found}, status=404)

        project_details_object = ProjectDetails.objects.filter(identifier=project).first()
        ...
        if project_details_object is None:
            ...
            ProjectDetails.objects.filter(pk=project).update(**data)
        return Response({'status': True, 'result': True}, status=200)
    except Exception as error:
        if created is True:
...
def news(request):
    ...
    try:
        data = dict(request.data)
        data['active'] = True
        project = Projects.objects.filter(pk=data.get('project_identifier')).first()
        news_item_object = News.objects.filter(identifier=data.get('identifier'),
                                               project_identifier=project).first()
        if news_item_object is None:
            data['project_identifier'] = project
        ...
        News.objects.filter(identifier=data.get('identifier'),
                            project_identifier=project).update(**data)
