...
<template>
  <section class="section">
    ...
    <b-field v-if="selected_project_manager != null">
      <b-table
        :data="projects"
    ...
    <b-field v-if="selected_project_manager != null" class="align-left">
      <b-button class="is-primary" @click="save()">
        <span style="font-weight: 600">Bijwerken / Opslaan</span>
      </b-button>

      <b-button class="is-danger has-padding" @click="remove()">
...
export default {
  ...
  computed: {
    filteredDataObj() {
      return this.project_managers.filter((option) => {
        return option.email.toString().toLowerCase().indexOf(this.name.toLowerCase()) >= 0
      })
    },
  },
  watch: {
    ...
    selected_project_manager: {
      handler() {
        ...
        for (let i = 0; i < this.projects.length; i++) {
          if (
            Object.prototype.hasOwnProperty.call(this.projects[i], 'id') &&
            this.selected_project_manager !== null
          ) {
            if (this.selected_project_manager.projects.includes(this.projects[i].id)) {
  ...
  methods: {
    ...
    handleSelected(option) {
      this.selected_project_manager = option
    },
    showAddPM() {
    ...
    create_pdf(manager_key) {
      ...
      doc.text('https://api-backend.app-amsterdam.nl/omgevingsmanager/' + manager_key, 10, textY)
      doc.save(name + '.pdf')
    },
    save() {
      let message =
        '<h1 style="padding-top:10px;padding-bottom:10px;">U staat op het punt om het account voor <b>' +
        this.selected_project_manager.email +
        '</b> toe te voegen / bij te werken.</h1>'
      message +=
        '<div style="margin-bottom: 15px;"><p>U heeft de onderstaande projecten geselecteerd:</p></div>'
      ...
      for (let i = 0; i < this.selected_projects.length; i++) {
        this.selected_project_manager.projects.push(this.selected_projects[i].id)
    ...
    remove() {
      let message =
        '<h1 style="padding-top:10px;padding-bottom:10px;">U staat op het punt om het account voor <b>' +
        this.selected_project_manager.email +
        '</b> te verwijderen. Deze actie kan niet ongedaan gemaakt worden.</h1>'
