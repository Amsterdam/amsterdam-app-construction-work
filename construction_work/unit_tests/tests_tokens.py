...
class SignInViewTest(BaseTokenTest):
    ...
    def test_wrong_username(self):
        """test wrong username"""
        response = self.client.post("/api/v1/get-token/", {"username": "wrong", "password": self.password})
    ...
    def test_wrong_password(self):
        """test wrong password"""
        response = self.client.post("/api/v1/get-token/", {"username": self.username, "password": "wrong"})
        result = json.loads(response.content.decode("utf-8"))
        expected_result = {"detail": "No active account found with the given credentials"}
        self.assertDictEqual(result, expected_result)
...
class RefreshTokenViewTest(BaseTokenTest):
    ...
    def test_correct(self):
        ...
        credentials = self.client.post("/api/v1/get-token/", {"username": self.username, "password": self.password})
