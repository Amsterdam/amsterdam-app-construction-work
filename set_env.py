...
class SetEnv:
    ...
    def weak(password):
        ...
        if (
            ...
            and any(x.isnumeric() for x in password)
        ):
            return False
    ...
    def questions(self):
        ...
        try:
            ...
            while True:
                ...
                postgres_database = (
                    input("Please enter your POSTGRES database (default: main_application): ") or "main_application"
                )
                postgres_user = input("Please enter your POSTGRES username (default: backend): ") or "backend"
                postgres_password = (
                    input(f"Please enter your POSTGRES password (or use: {random_psql_password}): ")
                    or random_psql_password
                )
                web_username = (
                    input("Please enter your WEB username (default: redactie@amsterdam.nl): ")
                    or "redactie@amsterdam.nl"
                )
                web_password = (
                    input(f"Please enter your WEB password (or use: {random_web_password}): ") or random_web_password
                )
